...
**Permission.** The shell makes no `CGPreflightScreenCaptureAccess` or `CGRequestScreenCaptureAccess` calls. Screen Recording cannot be granted in-process, and a fresh grant needs a relaunch, so the Capturer reports a three-way state and the shell only renders it. The Relaunch button can poll the side-effect-free `capturer.permissionState` to enable itself. During development every re-signed build resets the grant.
